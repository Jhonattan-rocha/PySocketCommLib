...
class Client:
    ...
    async def sync_crypt_key(self):
        key_to_send = await self.crypt.async_crypt.async_public_key_to_bytes()
        self.writer.write(key_to_send)
        await self.writer.drain()

        enc_key = await self.reader.read(2048)
        key = await self.crypt.async_crypt.async_decrypt_with_private_key(enc_key)
        await self.crypt.sync_crypt.async_set_key(key)
